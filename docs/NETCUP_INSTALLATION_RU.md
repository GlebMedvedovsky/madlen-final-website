...
> Актуальное продолжение после установленного preview flow `88e1af1`: [единый выпуск NETCUP_RELEASE_RU.md](NETCUP_RELEASE_RU.md). Для production использовать только новый release kit и эту последовательность. Разделы ниже сохраняются как история установки и прежних overlay; повторно их на действующем app не выполнять. Новый rollback также согласует статусы проектов, сохраняя их текущие тексты и медиа.
